    ...
    (function(self) {
        if (config.fx == "w") {
            ...
            for (var n = 0; n < number; n++) {
                ...
                var lines_data = { mx: mxx, lx: mxx - config.width, my: config.my, ly: config.ly, vx: config.vx, vy: config.vy, }
                self.lines.push(lines_data);
                var lines_data2 = { mx: mxx - config.width - config.jiange, lx: mxx - config.width - config.jiange - config.width / 2, my: config.my, ly: config.ly, vx: config.vx, vy: config.vy, }
        ...
        if (config.fx == "h") {
            ...
            for (var n = 0; n < number; n++) {
                ...
                var lines_data3 = { mx: config.mx, lx: config.lx, my: myy, ly: myy - config.width, vx: config.vx, vy: config.vy, }
                self.lines.push(lines_data3);
                var lines_data4 = { mx: config.mx, lx: config.lx, my: myy - config.width - config.jiange, ly: myy - config.width - config.jiange - config.width / 2, vx: config.vx, vy: config.vy, }
...
Createline.prototype.begin = function(element, config) {
    ...
    // var canvasObj = document.getElementsByClassName(element)
    var self = this;
    var canvas = { line_w: config.line_w || 3, vx: config.vx || 0, vy: config.vy || 0, color: config.color || "blue", canvas_w: config.canvas_w || 0, canvas_h: config.canvas_h || 0, mx: config.mx || 0, my: config.my || 0, lx: config.lx || 0, ly: config.ly || 0, fx: config.fx || "w", width: config.width || 20, jiange: config.jiange || 10, }
    ...
    } else { console.log("当前环境不支持canvas"); return null; }
    setInterval(function() {
        self.createline(self, canvas);
        if (canvas.fx == "w") { self.updateline_w(self, canvas); }
...
Createline.prototype.updateline_w = function(self, canvas) {
    ...
    var cnt = [];
    for (var j = 0; j < self.lines.length; j++) { if (self.lines[j].lx > canvas.canvas_w) { cnt.push(j); } }
...
Createline.prototype.updateline_h = function(self, canvas) {
    ...
    var cnt = [];
    for (var j = 0; j < self.lines.length; j++) { if (self.lines[j].ly > canvas.canvas_h) { cnt.push(j); } }
    for (var k = 0; k < cnt.length; k++) { self.lines.splice(k, 1); }
}
Createline.prototype.addline = function(self, canvas) {
    ...
    if (canvas.fx == "w") {
        if (self.lines[length - 1].lx > 0) {
            ...
            var lxx = ww - jg;
            var data = { mx: ww, lx: lxx, my: canvas.my, ly: canvas.ly, vx: canvas.vx, vy: canvas.vy, }
    ...
    if (canvas.fx == "h") {
        if (length > 0) {
            if (self.lines[length - 1].ly > 0) {
                ...
                var ly = my - jg;
                var data = { mx: canvas.mx, lx: canvas.lx, my: my, ly: ly, vx: canvas.vx, vy: canvas.vy, }
                self.lines.push(data);
            }
        } else {
            var data = { mx: canvas.mx, my: canvas.my, lx: canvas.lx, ly: canvas.ly, vx: canvas.vx, vy: canvas.vy, }
            self.lines.push(data);
        }
    }
}